      SUBROUTINE GENETIC( THENRVAR, PC )
      IMPLICIT NONE
      REAL C_START_GET, C_END_GET, PC(*)
      INTEGER THENRVAR
      integer seed, i, status
      real ctrl(12), x(THENRVAR), f, FUNC
...
c
      seed=-1
c     (choosing -1 for seed = use default value which is 123456)
      call rninit2(seed)
c
c     Set control variables (-1 = use default value)
c
c    (Meaning: Population Number, Type: Integer, Legal Values: <= 128, Default Value: 100)
      ctrl(1)=-1
c
c    (Meaning: Number of Generations, Type: Integer, Legal Values: < Infinity, Default Value: 500)
      ctrl(2)=50
c      
c    (Meaning: Encoding Accuracy, Type: Integer, Legal Values: <= 6, Default Value: 5)
      ctrl(3)=-1      
c
c    (Meaning: Crossover Rate, Type: Real, Legal Values: 0.0 <= & <= 1.0, Default Value: 0.85)
      ctrl(4)=-1
c      
c    (Meaning: Mutation Mode, Type: Integer, Legal Values: 2 or 3 or 4 or 5 or 6, Default Value: 2)
      ctrl(5)=-1
c      
c    (Meaning: Initial Mutation Rate, Type: Real, Legal Values: 0.0 <= & <= 1.0, Default Value: 0.005)
      ctrl(6)=-1
c      
c    (Meaning: Minimum Mutation Rate, Type: Real, Legal Values: 0.0 <= & <= 1.0, Default Value: 0.0005)
      ctrl(7)=-1
c      
c    (Meaning: Maximum Mutation Rate, Type: Real, Legal Values: 0.0 <= & <= 1.0, Default Value: 0.25)
      ctrl(8)=-1
c      
c    (Meaning: Fitness Differential, Type: Real, Legal Values: 0.0 <= & <= 1.0, Default Value: 1)
      ctrl(9)=-1
c      
c    (Meaning: Reproduction Plans, Type: Integer, Legal Values: 1 or 2 or 3, Default Value: 1)
      ctrl(10)=-1
c      
c    (Meaning: Elitism, Type: Integer, Legal Values: 0 or 1, Default Value: 1)
      ctrl(11)=-1
c      
c    (Meaning: Verbose Mode, Type: Integer, Legal Values: 0 or 1 or 2, Default Value: 0)
      ctrl(12)=-1
c
c     Now call pikaia
      call pikaia(FUNC,THENRVAR,ctrl,x,f,status)

      DO i = 1, THENRVAR
         PC(i) = C_START_GET(i)+x(i)*(C_END_GET(i)-C_START_GET(i))
      ENDDO
...
c*********************************************************************
      function FUNC(n,x)
c=====================================================================
c     Compute the fitness function
c=====================================================================
      implicit none
c
c     Input:
...
c     Output
      real FUNC
      DO i = 1, n
         y(i) = C_START_GET(i)+x(i)*(C_END_GET(i)-C_START_GET(i))
      ENDDO
      FUNC=-LOGMLFUNC(y,y)
...
c     Local
      ...
      external ran0
c
c     Common block to make iseed visible to rninit2 (and to save
c     it between calls)
      common /rnseed/ iseed
c
      urand = ran0( iseed )
