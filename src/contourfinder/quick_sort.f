C     ****f* contourfinder/SORTRX *
C
C   NAME
C     SORTRX() -- quick sort
C
C   DESCRIPTION
C     Here's a hybrid QuickSort.  It's based on suggestions in Knuth,
C     Volume 3, and performs much better than a pure QuickSort on short
C     or partially ordered input arrays.  
C
C   AUTHOR
C    Leonard J. Moss of SLAC
C
C   CREATION DATE
C     1986-07-15
C
C   USAGE
C     SORTRX(N,DATA,INDEX)
C
C   INPUTS
C     N    - INTEGER
C     DATA - REAL
...
C
C   HISTORY
C     Changed the vector INDEX to be a REAL vector. This is more suitable
C     for the contourfinder routine.
C
C     ***
...
C     The pivot key, DATAP, is then DATA[P].
      ...
      IF (DATA(INT(INDEX(L))) .GT. DATAP) THEN
         INDEX(P)=INDEX(L)
         INDEX(L)=REAL(INDEXP)
         INDEXP=INT(INDEX(P))
         DATAP=DATA(INDEXP)
      ...
      IF (DATAP .GT. DATA(INT(INDEX(R)))) THEN
         IF (DATA(INT(INDEX(L))) .GT. DATA(INT(INDEX(R)))) THEN
            INDEX(P)=INDEX(L)
            INDEX(L)=INDEX(R)
         ELSE
            INDEX(P)=INDEX(R)
...
C     the subsequence).
         ...
         I=I+1
         IF (DATA(INT(INDEX(I))).LT.DATAP) GOTO 300
...
C     the subsequence).
         ...
         J=J-1
         IF (DATA(INT(INDEX(J))).GT.DATAP) GOTO 400
...
C Q9: Straight Insertion sort
      ...
      DO 950 I=2,N
         IF (DATA(INT(INDEX(I-1))) .GT. DATA(INT(INDEX(I)))) THEN
            INDEXP=INT(INDEX(I))
            DATAP=DATA(INDEXP)
            P=I-1
  920       CONTINUE
               ...
               P=P-1
               IF (P.GT.0) THEN
                  IF (DATA(INT(INDEX(P))).GT.DATAP) GOTO 920
               ENDIF
            INDEX(P+1) = REAL(INDEXP)
         ENDIF
  950    CONTINUE
